...
parser.add_argument('-l', '--language-pair', help='Language pair', required=True)
...
langpair  = args.language_pair
language  = langpair.split('-')[0]

data_url  = storage_url_base + corpus + '/' + release + '/raw/' + language + '.zip'
data_file = corpus + '_' + release + '_raw_' + language + '.zip'

align_url  = storage_url_base + corpus + '/' + release + '/xml/' + langpair + '.xml.gz'
align_file = corpus + '_' + release + '_xml_' + langpair + '.xml.gz'

if not exists(align_file):
    sys.stderr.write(f"Downloading {align_url}\n")
    urllib.request.urlretrieve(align_url, align_file)
...
def print_document(pars):
    ...
    docCount += 1
    if json_encode:
        global corpus, document, release, language, max_pars
        while pars:
            if len(pars) >= max_pars:
                text = pars[:max_pars]
                pars = pars[max_pars:]
            else:
                text = pars
...
def align_start_element(name, attrs):
    global corpus
    if name == 'linkGrp':
        fromDoc = attrs['fromDoc'][:-3]
        filename = f"{corpus}/raw/{fromDoc}"
...
def parse_document(filename):
    global lzip, count, docCount, sentCount, errorCount
    global document, inSent, inPar, sents, pars, sentStr
...
with gzip.open(align_file,'r') as f:
    algparser = ParserCreate()
    algparser.StartElementHandler = align_start_element

    for line in f:
        algparser.Parse(line)
